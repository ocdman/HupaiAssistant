...
void CMainDlg::Thread_Normal(void *param) {
	...
	while(canNormal) {
		...
		ofn.nMaxFile = 512;
		ofn.hwndOwner = pWndIE->m_hWndOwner;
		...
		Tools::SaveBitmap3(ofn.lpstrFile, hBitmap);
...
void CMainDlg::Thread_AutoConfirm(void *param) {
	...
	timeb aTime;
	ftime(&aTime);
	sprintf(rl.m_cInfo,"写日志测试开始******************************+++++++++++++++++++++++_________________%s .%ld ms\n",ctime(&(aTime.time)),aTime.millitm);
	rl.WriteLogInfo(rl.m_cInfo);
	sprintf(rl.m_cInfo,"最晚出价时间：latest_time=%d，提交价：cmt_price=%d，延迟提交时间：cmt_delay=%d\n",latest_time,cmt_price,cmt_delay);
	rl.WriteLogInfo(rl.m_cInfo);
	...
	while(canAutoConfirm) {
		...
		sprintf(rl.m_cInfo,"当前时间：cur_time=%d\n",cur_time);
		rl.WriteLogInfo(rl.m_cInfo);
		// 到达最晚出价时间
		if(cur_time >= latest_time) {
			break;
		}
		sprintf(rl.m_cInfo,"当前最低价：price=%d\n",theApp.status.price);
		rl.WriteLogInfo(rl.m_cInfo);
		// 达到伏击价格
	...
	timeb bTime;
	ftime(&bTime);
	sprintf(rl.m_cInfo,"写日志测试结束******************************+++++++++++++++++++++++_________________%s .%ld ms\n",ctime(&(bTime.time)),bTime.millitm);
	rl.WriteLogInfo(rl.m_cInfo);
